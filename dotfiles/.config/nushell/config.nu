...
$env.config = {
  ...
  menus: [
    ...
    }
    {
      ...
      source: { |buffer, position|
        fd -E .git -E .jj -E .direnv -E node_modules -d 4 -H -t f -t l -L
        | lines
        | where ($it =~ $buffer)
        | each { |it| {value: $it} }
      }
    }
    {
      ...
      source: { |buffer, position|
        fd -E .git -E .jj -E .direnv -E node_modules -d 4 -H -t d -t l -L
        | lines
        | where ($it =~ $buffer)
        | each { |it| {value: $"./($it)"} }
      }
  ...
  keybindings: [
    ...
    }
    {
      ...
      event: [
        ...
        { send: executehostcommand cmd: "cd $'(fd -uuu -E .git -E .jj -E .direnv -E node_modules -d 4 -H -t d -t l -L | fzf --color $'(cat ~/.config/colorscheme),hl:1' --reverse --height 40%)'"}
      ]
    }
    {
      name: last_word_menu
    ...
    {
      # Inspired by https://github.com/nushell/nushell/issues/11555#issuecomment-1895771935
      name: reload_config
      modifier: none
      keycode: f5
      mode: [emacs vi_normal vi_insert]
