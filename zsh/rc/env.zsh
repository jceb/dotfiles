...
for i in "${HOME}/Documents/toolshed/" "${HOME}/.cabal/bin" "${HOME}/.local/bin" "${HOME}/bin" "${HOME}/.gem/ruby/2.3.0/bin" "${GOPATH}/bin" "${GOBIN}" "${HOME}/node_modules/.bin" "${HOME}/.cargo/bin" "${ANDROID_HOME}/tools"; do
	if [ -e "$i" ]; then
		PATH="$i:${PATH}"
	fi
done
