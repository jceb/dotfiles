...
$env.PATH = ($env.PATH | prepend [$"($env.HOME)/.rustup/toolchains/stable-x86_64-unknown-linux-gnu/bin" $"($env.HOME)/.krew/bin" $"($env.HOME)/.config/npm-global/bin" $"($env.HOME)/.deno/bin/" $"($env.HOME)/.cargo/bin"])
